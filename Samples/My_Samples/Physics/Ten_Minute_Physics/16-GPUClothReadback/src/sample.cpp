...
void SampleRenderPath::Render() const
{
	...
	// For each cloth object in the physics scene...
    for (auto& object : gPhysicsScene.objects)
    {
		// Process any pending readback results for positions and normals from the GPU.
        ...
        if (!gPhysicsScene.paused)
        {
			// Request new readbacks for positions and normals from the GPU for the next frame.
            object->cloth->RequestPositionsReadback(cmd);
            object->cloth->RequestNormalsReadback(cmd);
        }

		// Update the wireframe mesh with the latest CPU positions and normals from the cloth simulation.
