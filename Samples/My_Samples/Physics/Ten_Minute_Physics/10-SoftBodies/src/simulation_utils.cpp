...
std::unique_ptr<SoftBodyInstance> create_softbody_instance(const SoftBodyParams &params)
	{
		...
		GetScene().transforms.Create(entity);
		...
		// A subset specifies a range of indices to describe part of the mesh to be rendered with a specific material.
		...
		for (size_t t = 0; t < numTris; ++t)
		{
			...
			// The indices are relative to the vertex_position array.
		...
		// containing all mesh data of the scene.
