...
static float camera_pos[3] = {0.5f, 1.0f, -1.0f};	// Camera starting position
static float camera_ang[3] = {15.0f, -30.0f, 0.0f};	// Camera starting orientation (15 degrees down, 30 degrees to the left)
wi::scene::TransformComponent camera_transform;

static const std::string modelPath = "models/cloth.txt";
...
void init_wicked_scene()
{
	...
	//PointLight
	{
		auto lightEntity = scene.Entity_CreateLight("PointLight", XMFLOAT3(0, 2, -2));
...
void create_custom_shader()
{
	...
	static RasterizerState rs_vismesh;
	rs_vismesh.fill_mode = FillMode::SOLID;
	rs_vismesh.cull_mode = CullMode::NONE;
	rs_vismesh.front_counter_clockwise = true;
	rs_vismesh.depth_clip_enable = true;
	...
	static RasterizerState rs_wiremesh;
	rs_wiremesh.fill_mode = FillMode::WIREFRAME;
	rs_wiremesh.cull_mode = CullMode::NONE;
	rs_wiremesh.front_counter_clockwise = true;
	...
	// Register custom shaders and store their IDs for later use in the physics simulation initialization.
	vis_shader_id = RegisterCustomShader(customShader);
...
void init_gui(SampleRenderPath &srp)
{
    ...
    label.Create("Label1");
    label.SetText("Ten Minute Physics: 14 - Cloth Simulation");
	...
	restart.OnClick(
		[&](wi::gui::EventArgs args)
		{
			...
			{
				auto meshComponent = wi::scene::GetScene().meshes.GetComponent(object->wireEntity);
				meshComponent->DeleteRenderData();

				auto visMeshComponent = wi::scene::GetScene().meshes.GetComponent(object->visEntity);
				visMeshComponent->DeleteRenderData();
	...
	// newBody.SetPos(XMFLOAT2(90, 200));
	// newBody.OnClick(
	// 	[&](wi::gui::EventArgs args)
	// 	{
	// 		simulation::new_simulation_object(modelPath);
	// 	});
	...
	show_tetMesh.SetPos(XMFLOAT2(270, 200));
	show_tetMesh.OnClick(
		[&](wi::gui::EventArgs args)
		{
			for (size_t i = 0; i < gPhysicsScene.objects.size(); i++)
			{
				auto objTriComponent = wi::scene::GetScene().objects.GetComponent(gPhysicsScene.objects[i]->wireEntity);
				objTriComponent->SetRenderable(args.bValue);

				auto objVisComponent = wi::scene::GetScene().objects.GetComponent(gPhysicsScene.objects[i]->visEntity);
				objVisComponent->SetRenderable(!args.bValue);
			}
		});
	gui.AddWidget(&show_tetMesh);

    compliance.Create(0, 10, 1, 1, "slider1");
    compliance.SetText("Compliance: ");
    compliance.SetSize(XMFLOAT2(200, 20));
    compliance.SetPos(XMFLOAT2(90, 230));
	compliance.SetValue(0);
    compliance.OnSlide(
        [](wi::gui::EventArgs args)
        {
            for (size_t i = 0; i < gPhysicsScene.objects.size(); i++)
                gPhysicsScene.objects[i]->wireMesh->bendingCompliance = args.fValue;
        });
	...
	label_tris.SetPos(XMFLOAT2(90, 260));
	...
	label_verts.SetPos(XMFLOAT2(90, 290));
...
void SampleRenderPath::Update(float dt)
{
	...
	for (auto &object : gPhysicsScene.objects)
	{
		auto tetMeshComponent = wi::scene::GetScene().meshes.GetComponent(object->wireEntity);
		simulation::update_tetMesh(*object->wireMesh, *tetMeshComponent, true);
    ...
    if (wi::input::Press(wi::input::MOUSE_BUTTON_LEFT))
    {
        ...
        for (auto &object : gPhysicsScene.objects)
        {
            if (pick.entity == object->wireEntity || pick.entity == object->visEntity)
            {
                grabber.start(pick, object->wireMesh.get());
                mouse_down = true;
