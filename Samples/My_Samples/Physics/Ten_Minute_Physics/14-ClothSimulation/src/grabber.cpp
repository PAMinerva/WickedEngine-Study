...
void Grabber::start(wi::scene::PickResult pick, WireMesh* wireMesh)
{
    if (isGrabbing || wireMesh == nullptr)
        return;

    pickInfo = pick;
    this->wireMesh = wireMesh;

	this->wireMesh->StartGrab(pick);
...
void Grabber::move(wi::primitive::Ray ray, wi::scene::PickResult pick)
{
    ...
    this->wireMesh->MoveGrabbed(pos, vel);
}
...
void Grabber::end()
{
    if (isGrabbing && wireMesh != nullptr)
    {
        ...
        wireMesh->EndGrab(posVec, velVec);
        wireMesh = nullptr;
        isGrabbing = false;
    }
}
