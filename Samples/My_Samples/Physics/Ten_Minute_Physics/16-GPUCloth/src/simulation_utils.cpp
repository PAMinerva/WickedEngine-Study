...
void create_wire_mesh(std::unique_ptr<SimulationObject>& instance, const SimulationParams& params)
{
    ...
    // Setup vertex positions (float4 → float3 conversion)
...
void update_mesh(const ClothMesh& cloth, wi::scene::MeshComponent& mesh, wi::graphics::CommandList cmd)
{
    ...
    // Update streamout position buffer
    if (mesh.so_pos.IsValid())
    {
        wi::vector<wi::scene::MeshComponent::Vertex_POS32W> verts(cloth.numParticles);
        for (int i = 0; i < cloth.numParticles; i++)
            verts[i].FromFULL(XMFLOAT3(cloth.cpuPos[i].x, cloth.cpuPos[i].y, cloth.cpuPos[i].z), 0xFF);

        device->UpdateBuffer(
            &mesh.streamoutBuffer,
            verts.data(),
            cmd,
            verts.size() * sizeof(wi::scene::MeshComponent::Vertex_POS32W),
            mesh.so_pos.offset
        );
    }

    // Update streamout normal buffer
    if (mesh.so_nor.IsValid())
    {
        ...
        device->UpdateBuffer(
            ...
            nors.data(),
            cmd,
            nors.size() * sizeof(wi::scene::MeshComponent::Vertex_NOR),
            mesh.so_nor.offset
        );
    }
}
