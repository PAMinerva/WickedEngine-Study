...
// Simulation parameters (matching HTML defaults)
...
public:
    ...
    std::vector<XMFLOAT4> cpuPos;       // Current positions (for readback)
    std::vector<float> cpuInvMass;       // Inverse mass (for grab)
    std::vector<XMFLOAT4> restPos;       // Rest positions (for reset)
    std::vector<XMFLOAT3> cpuNormals;    // Normals (for readback → mesh update)

    // =====================================================================
    // Constraint data — 5 passes matching HTML
    // =====================================================================
    ...
    std::vector<uint32_t> constIds;      // Flat pairs: [2*i]=id0, [2*i+1]=id1
