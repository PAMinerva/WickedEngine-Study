...
void init_simulation(/*uint64_t visShaderID,*/ uint64_t wireShaderID, const std::string& modelPath)
{
    ...
    // params.rotate = {0.0f, 0.0f, 3.14f}; // rotate 180 degrees around Y axis clockwise (180 x (pi/180) = pi = 3.14 rad) (Yaw in Roll-Pitch-Yaw rotation)
    // params.edgeCompliance = 0.0f;
    // params.volCompliance = 0.0f;

    auto instance = create_softbody_skinning(params, modelPath);
                    ...
                    wi::scene::MeshComponent &mesh, bool updateGPUBuffer)
{
    ...
    size_t numTris = softbody.visMesh->numIndices / 3;
    ...
    for (size_t t = 0; t < numTris; ++t)
    {
		...
		uint32_t i0 = softbody.visMesh->indices[t * 3 + 0];
		uint32_t i1 = softbody.visMesh->indices[t * 3 + 1];
		uint32_t i2 = softbody.visMesh->indices[t * 3 + 2];
...
void create_tetMesh(std::unique_ptr<SoftBodySkinning> &instance, const SoftBodySkinningParams &params)
{
    ...
    tetMesh.subsets.back().indexCount = (uint32_t)getTetMesh().tetEdgeIds.size();
    ...
    instance->tetMesh = std::make_unique<TetraMesh>(getTetMesh(), params.edgeCompliance, params.volCompliance);
	...
	size_t numSurfTriInds = getTetMesh().tetEdgeIds.size();
    ...
    {
        tetMesh.indices[i] = (uint32_t)getTetMesh().tetEdgeIds[i];
    }
...
wi::ecs::Entity loadWiSceneModel(const std::string& model_path)
{
    wi::scene::Scene &scene = wi::scene::GetScene();
	wi::ecs::Entity model_entity = wi::scene::LoadModel(scene, model_path);
	return model_entity;
}

VisualMesh getVisualMesh(const std::string& model_path)
{
    VisualMesh visMesh;
    wi::scene::Scene& scene = wi::scene::GetScene();
	std::pair<wi::ecs::Entity, wi::ecs::Entity> modelEntities = ImportModel_OBJ(model_path, scene, true);

    if (modelEntities.first != wi::ecs::INVALID_ENTITY && modelEntities.second != wi::ecs::INVALID_ENTITY)
    	{
			wi::scene::ObjectComponent* obj = scene.objects.GetComponent(modelEntities.second);
    		wi::scene::MeshComponent* mesh = scene.meshes.GetComponent(obj->meshID);
    		if (mesh != nullptr) {
    			visMesh.rootEntity = modelEntities.first;
				visMesh.objEntity = modelEntities.second;
				visMesh.meshEntity = obj->meshID;
    			for (const auto& v : mesh->vertex_positions)
    				visMesh.verts.push_back(v);
    			for (const auto& idx : mesh->indices)
    				visMesh.indices.push_back(idx);
    		}
    		return visMesh;
    	}
...
void create_visMesh(std::unique_ptr<SoftBodySkinning> &instance, const SoftBodySkinningParams &params, const std::string &modelPath)
{
	using namespace wi::scene;
	using namespace wi::ecs;

	instance->visMesh = std::make_unique<VisualMesh>(getVisualMesh(modelPath));
	instance->visEntity = instance->visMesh->objEntity;
	instance->visMesh->numVerts = instance->visMesh->verts.size();
	instance->visMesh->indices = instance->visMesh->indices;
	instance->visMesh->numIndices = instance->visMesh->indices.size();
	instance->visMesh->numTris = instance->visMesh->numIndices / 3;

	instance->tetMesh->skinningInfo.resize(instance->visMesh->verts.size(), XMFLOAT4(0, 0, 0, 0));
	...
	auto visMeshObj = GetScene().objects.GetComponent(instance->visEntity);
	auto visMesh = *wi::scene::GetScene().meshes.GetComponent(visMeshObj->meshID);
	...
	{
		visMesh.indices[t] = instance->visMesh->indices[t];
	}
    ...
    for (size_t t = 0; t < numTris; ++t)
    {
		...
		// NOTE: Vertex winding order and normal calculation
		//
		// Problem:
		// - The original OBJ model defines triangle vertices in clockwise order
		// - WickedEngine uses by default a rasterizer with counter-clockwise front faces
		// - ImportModel() does change the winding order to counter-clockwise when loading OBJ files
		//   by negating the z coordinate of vertices and normals if the last argument of the function is true.
		//
		// Engine implementation details:
		// - In wiRenderer.cpp, LoadShaders() configures ObjectRenderingVariant with front_counter_clockwise = true
		//   (see the section where desc.rs is set, especially the default case)
		// - In wiRenderer.cpp, RenderMeshes() sets cullmode to BACK when doubleSided is false
		//   (for both mesh and material, which is the default when loading models)
		// - Entity_CreateMesh() does not set doubleSided to true by default
		// - Result: rasterizers[RSTYPE_FRONT] is used with front_counter_clockwise = true
		//
		// Solution:
		// To correctly calculate the normal, the first triangle edge must go towards the second
		// in counter-clockwise order. Therefore in the cross product:
		// - First vector: (v2 - v0)
		// - Second vector: (v1 - v0)
		//
		// ps. This is the opposite of the order used in previous samples, where the mesh triangles were defined in clockwise winding.
		XMVECTOR n = XMVector3Cross(v2 - v0, v1 - v0);
		n = XMVector3Normalize(n);
...
std::string getTetMeshPath(const std::string& modelPath)
{
    size_t dot = modelPath.find_last_of('.');
    if (dot == std::string::npos)
        return modelPath + "_tet.txt";
    return modelPath.substr(0, dot) + "_tet.txt";
}
...
std::unique_ptr<SoftBodySkinning> create_softbody_skinning(const SoftBodySkinningParams &params, const std::string &modelPath)
{
	...
	gTetMeshPath = getTetMeshPath(modelPath);

    create_tetMesh(instance, params);
    create_visMesh(instance, params, modelPath);
...
void remove_softbody_skinning(SoftBodySkinning &sbi)
{
	...
	// Remove tetrahedral mesh components
	...
	// Remove visual mesh components
	GetScene().hierarchy.Remove(sbi.visMesh->rootEntity);
	GetScene().objects.Remove(sbi.visMesh->objEntity);
	GetScene().meshes.Remove(sbi.visMesh->meshEntity);
}
} // namespace simulation
