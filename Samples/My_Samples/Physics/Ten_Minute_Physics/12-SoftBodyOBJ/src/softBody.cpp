...
VisualMesh::VisualMesh(const VisMesh& visMesh)
{
	numVerts = static_cast<int>(visMesh.verts.size() / 3);
	numTris = static_cast<int>(visMesh.triIds.size() / 3);
	...
	for (int i = 0; i < numTris; ++i) {
		XMINT3 t(
			visMesh.triIds[3 * i + 0],
			visMesh.triIds[3 * i + 1],
			visMesh.triIds[3 * i + 2]
		);
		triIds.push_back(t);
	}
}
...
TetraMesh::TetraMesh(
	...
	, grads(4 * 3, 0.0f)
{
	// skinningInfo.resize(getDragonVisMesh().verts.size() / 3, XMFLOAT4(0, 0, 0, 0));
...
void TetraMesh::Translate(const XMFLOAT3& delta)
{
...
void TetraMesh::Scale(const XMFLOAT3& scale)
{
...
void TetraMesh::RotateRollPitchYaw(const XMFLOAT3& angle)
{
...
float TetraMesh::GetTetVolume(int nr)
{
...
void TetraMesh::PreSolve(float dt, const XMFLOAT3& gravity)
{
...
void TetraMesh::SolveEdges(float compliance, float dt)
{
...
void TetraMesh::SolveVolumes(float compliance, float dt)
{
    float alpha = compliance / (dt * dt);
...
void TetraMesh::Solve(float dt)
{
    SolveEdges(edgeCompliance, dt);
    SolveVolumes(volCompliance, dt);
}

void TetraMesh::PostSolve(float dt)
{
    for (int i = 0; i < numVerts; i++)
    {
		// skip fixed vertices
...
void TetraMesh::StartGrab(const wi::scene::PickResult pick)
{
...
void TetraMesh::MoveGrabbed(const std::vector<float>& pos, const std::vector<float>& vel)
{
    if (grabId >= 0)
    {
...
void TetraMesh::EndGrab(const std::vector<float>& pos, std::vector<float>& vel)
{
    if (grabId >= 0)
    {
