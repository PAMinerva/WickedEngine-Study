...
std::pair<wi::ecs::Entity, wi::ecs::Entity> ImportModel_OBJ(const std::string& fileName, Scene& scene, bool transform_to_LH)
{
	...
	if (success)
	{
		...
		{
			objectEntity = scene.Entity_CreateObject(shape.name);
		...
		return std::make_pair(rootEntity, objectEntity);
	}
	else
	{
		wi::helper::messageBox("OBJ import failed! Check backlog for errors!", "Error!");
		return std::make_pair(INVALID_ENTITY, INVALID_ENTITY);
	}
}
