...
void Grabber::start(wi::scene::PickResult pick, TetraMesh* tetMesh)
{
    if (isGrabbing || tetMesh == nullptr)
        return;

    pickInfo = pick;
    this->tetMesh = tetMesh;

	this->tetMesh->StartGrab(pick);
...
void Grabber::move(wi::primitive::Ray ray, wi::scene::PickResult pick)
{
    ...
    this->tetMesh->MoveGrabbed(pos, vel);
}
