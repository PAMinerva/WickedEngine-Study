...
static float camera_pos[3] = {0.0f, 1.0f, -3.0f}; // Camera starting position
static float camera_ang[3] = {8.0f, 0.0f, 0.0f};  // Camera starting orientation (8 degrees down)
...
void init_gui(SampleRenderPath &srp)
{
    ...
    label.Create("Label1");
    label.SetText("Ten Minute Physics: 12 - Soft Body Skinning");
	...
	restart.OnClick(
		[&](wi::gui::EventArgs args)
		{
			...
			simulation::init_physics(vis_shader_id, wire_shader_id);
		});
	...
	newBody.OnClick(
		[&](wi::gui::EventArgs args)
		{
			simulation::new_softbody_skinning();
		});
